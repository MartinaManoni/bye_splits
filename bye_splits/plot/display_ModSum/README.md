...
Partial trigger towers are constructed at Stage 1 from raw Module Sums without any energy suppression or STCs. These trigger towers are static, geometrically defined objects spanning a fixed 20×72 bin grid in the 𝜂–φ plane, covering the region 𝜂 ∈ [1.5, 3] with a granularity of about 5° in φ and ≈0.08 in 𝜂. This granularity closely matches the existing CMS calorimeter trigger towers.

This project focuses on:

- Performing comprehensive studies across multiple particle types—**photons, pions, jets**—under **zero and 200 pile-up conditions**, along with **minimum bias**, samples for trigger rate analysis.
...
1. Ingest ROOT ntuples
2. Assign Module sums/STCs to fixed η–φ tower grids using several algorithms (baseline, area‑overlap, 4/8/16‑tower splitting logics).
4. Match reconstructed particles to generator particles, evaluate energy and position resolution in η/φ for performance studies.
...
| `--n`                | *None*     | Process *n* events randomly selected events                                                |
...
`/data_CMS/cms/manoni/L1HGCAL/final_skimmed_V16ntuples/` for samples containing only Modules Sums and in 
`/data_CMS/cms/manoni/L1HGCAL/final_skimmed_V16ntuples_STCS/` for samples containing Module Sums and STCs.
Samples used at the moemnt are hardcoded in `mainModulesSums.py`
...
* **Terminal prints**: per‑event debug, summary of towers, resolutions.
* **Plots** (if enabled via downstream calls).
